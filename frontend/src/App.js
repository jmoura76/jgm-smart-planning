...
function App() {
  ...
  const inventoryChartData = kpis
    ? [
        { name: "Risco", value: kpis.materiais_risco },
        { name: "Excesso", value: kpis.materiais_excesso },
      ]
    : [];

  const ordersChartData = kpis
    ? [
        { name: "Total OPs", value: kpis.total_ops },
        { name: "Atrasadas", value: kpis.ops_atrasadas },
      ]
    : [];
  ...
  const capacityPieData =
    capacidade && capacidade.total_recursos > 0
      ? [
          { name: "< 90%", value: capacidade.recursos_abaixo_90 },
          { name: "90–100%", value: capacidade.recursos_90_100 },
          { name: "> 100%", value: capacidade.recursos_acima_100 },
        ]
      : [];
    ...
    <div className="app">
      ...
      <div className="app-body">
        ...
        <main className="main-panel">
            ...
            <>
              <section className="cards-row">
                ...
                <KpiCard
                  ...
                  subtitle="Base COHV"
                  type="danger"
                  progress={kpis.perc_ops_atrasadas}
                />
              </section>
          ...
          {/* ------------------------------------------------------------------ */}
          {/* PLANEJAMENTO IA – usa apenas o componente (sem título duplicado)   */}
          {/* ------------------------------------------------------------------ */}
          {!loading && !error && data && activeTab === "planning" && (
            <section className="planning-section">
              <PlanningBoardIa />
            </section>
          )}
          ...
          {/* ------------------------------------------------------------------ */}
          {/* PEGGING IA – página separada, sem título duplicado                 */}
          {/* ------------------------------------------------------------------ */}
          {!loading && !error && activeTab === "pegging" && (
            <section className="planning-section pegging-page">
              <PeggingIaLite />
            </section>
          )}
        </main>
      ...
      <footer className="app-footer">
        <span>© 2025 JGM – SmartPlanning™</span>
        <span className="footer-separator">|</span>
        <span>
          Powered by <strong>IA Engine™</strong>
        </span>
      </footer>
    </div>
  );
...
function KpiCard({ title, value, subtitle, type, progress }) {
